...
class ReST(object):
	...
	def preprocess(self, hvg_prop=0.8,species='Human', n_pcs=30):
...
class ReST(object):
	...
	def extract_regions(self, min_sim=0.5, max_sim=0.91, gap=0.05, min_region=40, sigma=1):
		warnings.filterwarnings('ignore')
		...
		if mode == 'mutual':
			...
			for j in trange(len(regions)):
				...
				for i in range(len(genes)):
					...
					pvals.append(pval)
				padjs = multipletests(pvals)[1]
				df = pd.DataFrame({'region': region, 'gene': genes,'lfc': lfcs, 'pval': pvals, 'padj': padjs})
				df = df.loc[(df.lfc > 0.26) & (df.padj <= 0.01)]
				df = df.sort_values("lfc", ascending=False)
				region_marker_dict[region] = df
			self.region_vAll_marker_dict = region_marker_dict
	...
	def plot_region_enrichment(self):
		regions = self.region_enrichment_result.keys()
		region_dfs = []
		for region in regions:
			res = self.region_enrichment_result[region].sort_values(by=['Odds Ratio'], ascending=False).iloc[:10,:]
			res['region_ind'] = region
			region_dfs.append(res)
		region_dfs = pd.concat(region_dfs)
		region_dfs['-log10(P-adjust)'] =  - np.log10(10E-10 + region_dfs['Adjusted P-value'])
		region_dfs['log2(Odds-ratio)'] = np.log2(region_dfs['Odds Ratio']).astype('float')
		...
		fig = px.bar(region_dfs, x='-log10(P-adjust)', y="Term", 
			color="log2(Odds-ratio)", barmode="group",
			  facet_col="region_ind", height=800, width=1600)
		fig.show()
